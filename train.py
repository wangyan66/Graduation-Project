...
class MyCorpus:
    def __iter__(self):
        ...
        #     path = os.path.join(i,'text.txt')
            for line in open("data_test/text.txt"):
                yield line.split()
logging.basicConfig(format='%(asctime)s:%(levelname)s: %(message)s', level=logging.INFO)
sentences = MyCorpus()
# for v in sentences:
    # print(v)
model=gensim.models.Word2Vec(sentences,sg=1,vector_size=100,window=5,min_count=2,negative=3,sample=0.001,hs=1,workers=8, epochs=10)
